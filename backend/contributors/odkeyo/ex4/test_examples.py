...
class MonsterListRequestSchema:
    ...
    def load(self, data):
        ...
        if not isinstance(data, dict):
            raise ValidationError({"resource": ["Invalid payload"]})
...
class MonsterGetRequestSchema:
    ...
    def load(self, data):
        ...
        if not isinstance(data, dict):
            raise ValidationError({"monster_index": ["Invalid payload"]})
        # HINT: Check for "monster_index" field
        idx = data.get("monster_index")
        if idx is None:
        ...
        if not str(idx).strip():
            raise ValidationError({"monster_index": ["Must not be empty"]})
        if len(str(idx)) > 100:
            raise ValidationError({"monster_index": ["Length must be <= 100"]})
        # HINT: Raise ValidationError with proper field-specific messages
        return {"monster_index": str(idx)}
