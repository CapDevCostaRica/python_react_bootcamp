...
from app.common.python.common.schema.schema import ShipmentUpdateRequestSchema
...
def handler(event, context):
    ...
    with get_session() as session:
        ...
        try:
            schema = ShipmentUpdateRequestSchema(context={
                "role": role,
        ...
        if role == "carrier" and shipment.status == "in_transit" and shipment.assigned_carrier_id == user_id:
            if location:
                print(f"Tracking location added for shipment {shipment_id}: {location}")
