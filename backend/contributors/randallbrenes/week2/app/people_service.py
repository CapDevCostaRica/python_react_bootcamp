from app.models import People, Family, Favorite, Hobbies, Studies, Base
...
class PeopleService:
    def getFieldTable(self, field):
        ...
        tables = [People, Family, Favorite, Hobbies, Studies]
        for table in tables:
        ...
        try:
            with get_session() as db:
                stmt = (
                    select(
                        Favorite.food,
                        func.count(Favorite.food).label("n")
                    )
                    .group_by(Favorite.food)
                    .order_by(func.count(Favorite.food).desc())
                    .limit(1)
                )
    ...
    def extra2(self):
        try:
            with get_session() as db:
                ...
                results = db.execute(query).all()
            data = {f"{row.nationality}-{row.hair_color}": round(row.weight) for row in results}
            return self.okResponse(data)
        ...
        try:
            with get_session() as db:
                ...
                query = (
                    ...
                    .where(subq.c.age_rank <= 2).order_by(subq.c.nationality.asc(), subq.c.age_rank.asc())
                )
            ...
            with get_session() as db:
                query = select(
                    ...
                    func.count(Hobbies.hobby).label("hobbies")).group_by(
                        People.id
                    ).limit(3).order_by(func.count(Hobbies.hobby).desc())
    ...
    def extra5(self):
        try:
            with get_session() as db:
                avg_general_subquery = select(func.avg(People.age)).scalar_subquery()
                query = select(
                        People.nationality,
                        func.avg(People.age).label("avg_age"),
                        avg_general_subquery.label("avg_general_age")
                    ).group_by(
                        People.nationality
                    )
            ...
            data = {
                "general": round(results[0].avg_general_age) if results else 0,
                "nationalities": {row.nationality: round(row.avg_age) for row in results}
            }
