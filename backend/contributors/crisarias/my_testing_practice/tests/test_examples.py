...
class MonsterListRequestSchema:
    ...
    def load(self, data):
        if "resource" not in data:
            raise ValidationError("Missing resource field")
...
class MonsterGetRequestSchema:
    ...
    def load(self, data):
        ...
        if "monster_index" not in data:
            raise ValidationError({"monster_index": ["monster_index field is a required field."]})
        if not data["monster_index"] or len(data["monster_index"].strip()) == 0:
            raise ValidationError("monster_index cannot be empty")
        if len(data["monster_index"]) > 100:
            raise ValidationError("monster_index cannot be longer than 100 characters")
        return data  # This needs validation!
...
class TestMonsterAPI:
    ...
    def test_monsters_list_from_cache(self, client, db_session, sample_monster_data):
        ...
        """
        import conftest
        conftest._shared_session = db_session
