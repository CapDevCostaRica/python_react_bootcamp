...
class Monster(Base):
    ...
    def __init__(self, index=None, name=None, data=None):
        ...
        # Check if index and name are provided, raise ValueError if missing
        if index is None:
...
class MonsterListRequestSchema:
    ...
    def load(self, data):
        # Add validation for monster list requests
        # Check for "resource" field and validate it equals "monsters"
        # Raise ValidationError for missing or invalid resource
        from marshmallow import ValidationError
...
class MonsterGetRequestSchema:
    ...
    def load(self, data):
        ...
        # Check for "monster_index" field
        # Validate monster_index is not empty and not too long
        # Raise ValidationError with proper field-specific messages
        from marshmallow import ValidationError
        ...
        # Validate monster_index is not empty and not too long
        if not isinstance(monster_index, str):
            raise ValidationError({"monster_index": ["Not a valid string."]})

        # Check for empty string or whitespace only
        if not monster_index or not monster_index.strip():
            raise ValidationError({"monster_index": ["Field cannot be empty."]})

        # Check for too long (> 100 characters)
        if len(monster_index) > 100:
            raise ValidationError({"monster_index": ["Field cannot be longer than 100 characters."]})
